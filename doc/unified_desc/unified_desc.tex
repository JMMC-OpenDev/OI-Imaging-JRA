...
\begin{table}
  ...
  \resizebox{\textwidth}{!}{
    ...
    \begin{tabular}{P{6.2em}P{8.1em}P{9.3em}l}
      \TopRule
      \rowcolor{tableheadcolor}
      \bf Name & \bf Authors & \bf Optimization & \bf Regularization \\
      %\hline
      BSMEM & Baron, Buscher & Trust region gradient & MEM-prior \\
      MiRA & Thi{\'e}baut & VMLM-B & Many \\
      WISARD & Meimon, Mugnier, Le~Besnerais & VMLM-B plus\\self-calibration & Many \\
      %\hline
      MACIM & Ireland, Monnier & Simulated annealing & MEM \\
      SQUEEZE & Baron, Monnier, Kloppenborg & Parallel tempering & Sparsity \\
      %\hline
...
\begin{table}
  ...
  \resizebox{\textwidth}{!}{
    \rowcolors{3}{tablerowcolor}{}
    \begin{tabular}{M{8.2em}ccl}
      \TopRule
      \rowcolor{tableheadcolor}
      \bf Name & $\Fprior(\VParam)$ & \bf Parameters & \bf Implemented by \\
      ...
      w^\PriorTag_n = \norm{\VDirn_n}^\beta$ & $\beta > 0$ & \Mira, \Wisard \\
      %\hline
      Edge-preserving smoothness
      ($\sim\text{TV}$ when $\epsilon\!\rightarrow\!0$)
      & $\sum_{n_1,n_2}\sqrt{\epsilon^2 + \abs{\nabla\VParam}_{n_1,n_2}^2}$
      & $\epsilon > 0$ & \Mira, \Wisard \\
      \BottomRule
    \end{tabular}
  }
