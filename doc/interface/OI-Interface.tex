...
\oops{fitsverify gives a warning if \KEYWORD{CTYPEi} are
  omitted. Should these given as \STRING{RA} and \STRING{DEC},
  meaning linear axes, or as \STRING{RA---SIN} and \STRING{DEC--SIN}
  which specify the appropriate non-linear transformations between the
  tangent plane and the celestial sphere?}
...
\KEYWORD{HDUNAME} keyword).  \oops{OK but the last image should have a
  ...
  input parameters HDU with name of the final image and let all reconstructed
  images be called:
\begin{quote}
...
\oops{Implementing the additional columns for model data might not be
  so easy. When reading in the data, the imaging code must keep track
  of where in the OIFITS file each datum came from (HDU and wavelength
  channel) so that it can write the corresponding model datum in the
  correct place later. Also low-level access to the FITS file is
  required.

  An alternative would be to write additional data using the standard
  OIFITS columns, with a distinct TARGET\_ID. The extra table rows for
  model data could be appended to the existing tables or written to
  new tables. This approach would allow a standard OIFITS API to be
  used.}
